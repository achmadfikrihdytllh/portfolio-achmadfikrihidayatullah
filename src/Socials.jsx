...
const ITEMS = [
  {
    id: "instagram", label: "INSTAGRAM", handle: "@fikuri.olnyv", href: "https://www.instagram.com/fikuri.olnyv/", icon: "📷", barIcon: icon1, bars: 1, newBars: [0], counts: ["01"],
    links: ["www.instagram.com/fikuri.olnyv"],
    stats: [
      { tag: "FOL", value: "01", color: "#e1306c" },
      { tag: "PST", value: "24",  color: "#f77737" },
    ],
  },
  {
    id: "linkedin", label: "LINKEDIN", handle: "/in/achmad-fikri-hidayatullah-380197250", href: "https://www.linkedin.com/in/achmad-fikri-hidayatullah-380197250/", icon: "in", barIcon: icon2, bars: 1, newBars: [0], counts: ["01"],
    links: ["www.linkedin.com/in/achmad-fikri-hidayatullah-380197250"],
    stats: [
      { tag: "EXP", value: "02", color: "#e1306c" },
      { tag: "EDU", value: "UB",  color: "#f77737" },
    ],
  },
  {
    id: "portfolio", label: "PORTFOLIO", handle: "portfolio-p3r-fikuri", href: "https://portfolio-p3r-fikuri.vercel.app", icon: "↗", barIcon: icon3, bars: 1, newBars: [0], counts: ["01"],
    links: ["portfolio-p3r-fikuri.vercel.app"],
    stats: [
      { tag: "LIVE", value: "01", color: "#00f2ea" },
      { tag: "SYS", value: "07",  color: "#ff0050" },
    ],
  },
];
  ...
  useEffect(() => {
    const onKey = (e) => {
      if (focus === "left") {
        ...
        if (e.key === "Enter")      { playSound(selectItemSound); window.open(ITEMS[active].href, "_blank"); }
      } else {
        const barCount = ITEMS[active].bars;
        if (e.key === "ArrowUp")   { playSound(selectItemSound); setActiveInfoBar(i => Math.max(0, i - 1)); }
        if (e.key === "ArrowDown") { playSound(selectItemSound); setActiveInfoBar(i => Math.min(barCount - 1, i + 1)); }
        if (e.key === "ArrowLeft") setFocus("left");
        if (e.key === "Enter")     { playSound(selectItemSound); window.open("https://" + ITEMS[active].links[activeInfoBar], "_blank"); }
      }
      if ((e.key === "ArrowLeft" && focus === "left") || e.key === "Escape" || e.key === "Backspace") { playSound(backSound); navigate(-1); }
    };
